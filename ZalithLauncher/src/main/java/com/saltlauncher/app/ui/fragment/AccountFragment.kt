...
import com.saltlauncher.app.databinding.FragmentAccountBinding
import com.saltlauncher.app.databinding.ItemOtherServerBinding
import com.saltlauncher.app.databinding.ViewAddOtherServerBinding
...
import com.saltlauncher.app.feature.accounts.LocalAccountUtils.CheckResultListener
import com.saltlauncher.app.feature.accounts.LocalAccountUtils.Companion.checkUsageAllowed
import com.saltlauncher.app.feature.accounts.LocalAccountUtils.Companion.openDialog
import com.saltlauncher.app.feature.accounts.OtherLoginHelper
import com.saltlauncher.app.feature.log.Logging
import com.saltlauncher.app.feature.login.OtherLoginApi
import com.saltlauncher.app.feature.login.Servers
import com.saltlauncher.app.feature.login.Servers.Server
...
import com.saltlauncher.app.ui.dialog.OtherLoginDialog
import com.saltlauncher.app.ui.dialog.TipDialog
import com.saltlauncher.app.ui.layout.AnimRelativeLayout
import com.saltlauncher.app.ui.subassembly.account.AccountAdapter
import com.saltlauncher.app.ui.subassembly.account.AccountAdapter.AccountUpdateListener
import com.saltlauncher.app.ui.subassembly.account.AccountViewWrapper
import com.saltlauncher.app.ui.subassembly.account.SelectAccountListener
...
class AccountFragment : FragmentWithAnim(R.layout.fragment_account), View.OnClickListener {
    ...
    private fun defaultElyByServer(): Server = Server().apply {
        serverName = "Ely.by"
        baseUrl = "https://authserver.ely.by"
        register = "https://account.ely.by/register"
    }

    private fun ensureDefaultServers(config: Servers): Boolean {
        val servers = (config.server ?: ArrayList()).toMutableList()
        val hasEly = servers.any { it.baseUrl.equals("https://authserver.ely.by", true) }
        if (!hasEly) {
            servers.add(0, defaultElyByServer())
    ...
    private fun refreshOtherServer() {
        Task.runTask {
            ...
            val config = if (mOtherServerConfigFile.exists()) {
                runCatching {
                ...
                Servers().apply { info = "Default Salt Launcher servers" }
            }

            val updated = ensureDefaultServers(config)
            mOtherServerConfig = config
            config.server?.forEach { server ->
                mOtherServerList.add(server)
            }

            if (updated || !mOtherServerConfigFile.exists()) {
                Tools.write(
