...
class AccountFragment : FragmentWithAnim(R.layout.fragment_account), View.OnClickListener {
    ...
    private fun refreshOtherServer() {
        Task.runTask {
            ...
            // Ensure Ely.by is available by default
            val elyBaseUrl = "https://authserver.ely.by"
            if (mOtherServerList.none { it.baseUrl == elyBaseUrl }) {
                val elyServer = Server().apply {
                    serverName = "Ely.by"
                    baseUrl = elyBaseUrl
                    register = "https://account.ely.by/register"
                }
                checkServerConfig()
                mOtherServerConfig?.server?.add(elyServer)
                mOtherServerList.add(elyServer)
                Tools.write(
