...
public final class Settings {
    ...
    private Settings() {
        ...
        // Preload Ely.by auth server for convenience.
        try {
            if (config().getAuthlibInjectorServers().isEmpty()) {
                AuthlibInjectorServer ely = AuthlibInjectorServer.locateServer("https://authserver.ely.by");
                config().getAuthlibInjectorServers().add(ely);
            }
        } catch (Exception ignored) {
            // ignore failure; user can add manually
        }
